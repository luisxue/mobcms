...
class UsersAction extends BaseAction{
	...
	public function login(){		
		if(!isset($_POST['username'])){		
			...
			if($_SESSION['verify']!=md5(trim($_POST['code']))){			
			...
			if($res&&($pwd===$res['password'])){
				...
				if($res['status']==0){
					...
					//$this->success('您还没有审核您的账号,请扫描二维码审核帐号',C('site_url').'/shenhe.html');
					//exit;
				}
				session('uid',$res['id']);
				session('gid',$res['gid']);
				...
				$this->assign('err',array('err'=>0,'msg'=>'用户名或者密码不正确'));
				$this->display();
			}
		}	
	}
		...
		if(!isset($_POST['username'])){
			...
			if($data=$db->create()){
				...
				if($id){				
					...
					$this->assign('err',array('err'=>0,'msg'=>'注册失败，请重新填写您的信息'));
					$this->display();
				}
			}else{
				...
				$this->assign('err',array('err'=>0,'msg'=>$db->getError()));
				$this->display();			
			}		
		}		
	}	
